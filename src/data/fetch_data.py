...
def get_stock_and_sentiment():
    ...
    for symbol in SP100_TICKERS:
        ...
        for n in news:
            stock_sent_dict[pd.to_datetime(n['content']['pubDate']).strftime(
                '%Y-%m-%d')].append(get_sentiment(n['content']['title']))
        data = [{'Date': pd.to_datetime(k), 'sentiment': sum(
            stock_sent_dict[k]) / len(stock_sent_dict[k])} for k in stock_sent_dict]
        ...
        stock_sent_df = pd.merge(stock_df, stock_sent_df, on='Date', suffixes=(
